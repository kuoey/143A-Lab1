...
//for donating priority, keep track of locked list
//todo probs not need
//static struct lock sleep_list_lock; // added


int constant1, constant2;  //added
...
timer_init(void) {
    ...
    //initalize the sleep list and lock
    list_init(&sleep_list); //added
//    lock_init(&sleep_list_lock); //added

    //todo mlfqs
    constant1 = divide_fixed_and_integer(convert_to_fixed_point(59), 60); //added
    constant2 = divide_fixed_and_integer(convert_to_fixed_point(1), 60); //added
}
...
bool compareSleep(struct list_elem *a, struct list_elem *b, void *aux) {
    ...
    struct thread *tPointer1 = list_entry(a,
    struct thread, elem);
    struct thread *tPointer2 = list_entry(b,
    struct thread, elem);
