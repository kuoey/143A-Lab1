...
static struct list mlfqs_list[64]; //added
static int system_load_avg = 0; //added
...
static bool thread_priority_cmp(const struct list_elem *t1_,
    ...
    const struct thread *t1 = list_entry(t1_,
    struct thread, elem);
    const struct thread *t2 = list_entry(t2_,
    struct thread, elem);
...
thread_create(const char *name, int priority,
    ...
    tid_t tid;
    enum intr_level old_level; //keep track of the old level and prevents the process from being interrupted
    ...
    t->nice = thread_current()->nice; //get parent's nice value //added
    t->recent_cpu = thread_current()->recent_cpu; //get parent's recent_cpu value //added

    /* Initialize thread. */
    init_thread(t, name, priority);
    tid = t->tid = allocate_tid();

    //disable inerrupts for current program and returns the intr value of the current level
    ...
    //set it back to whatever it was before the disable
    ...
    //if t's prio
//    if (t->priority > thread_current()->priority)//added
//        thread_yield();//added
...
thread_yield(void) {
    ...
    if (cur != idle_thread) {
        if (!thread_mlfqs) //added
            ...
            list_push_back(&mlfqs_list[cur->priority], &cur->elem); //added
    }
...
thread_set_priority(int new_priority) {
    ...
    if (new_priority <= PRI_MAX && new_priority >= PRI_MIN) {
        //if the new priority is greater, then assign the new and old priority accordingly
        if (new_priority > thread_current()->priority) {
            thread_current()->priority = new_priority;
            thread_current()->old_priority = new_priority;
        } else if (thread_current()->priority == thread_current()->old_priority) {
            //if the priority is the same as old prio, that means nothing is donated
            thread_current()->priority = new_priority;
            thread_current()->old_priority = new_priority;
        } else {
            //if the donations is working on donation, saves new prio into old prio so that it can be used
        ...
        //yield accordingly
        t = list_entry(list_max(&ready_list, thread_priority_cmp, NULL),
...
//    intr_set_level (old_level);
    ...
    //list of what locks this current thread is holding
    list_init(&t->locks_held); //added
    //sets the lock to which this current thread is waiting on, initially null
    t->wait_on_lock = NULL; //added
}
...
next_thread_to_run(void) {
    if (!thread_mlfqs) {
        if (list_empty(&ready_list))
            ...
            //choose highest prio instead of popping
        ...
        while (i >= 0 && list_empty(&mlfqs_list[i]))
            i--;
        if (i >= 0)
            return list_entry(list_pop_front(&mlfqs_list[i]),
        struct thread, elem);
        else
        return idle_thread;
    }
